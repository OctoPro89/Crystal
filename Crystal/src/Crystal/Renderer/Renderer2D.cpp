...
namespace Crystal {
	...
	void Renderer2D::DrawQuad(const glm::mat4& transform, const Ref<SubTexture2D>& subtexture, float tilingFactor /*= 1.0f*/, const glm::vec4& tintColor /*= glm::vec4(1.0f)*/, int entityID /*= -1*/)
	{
		CRYSTAL_PROFILE_FUNCTION();
	...
	void Renderer2D::DrawCircle(const glm::mat4& transform, const glm::vec4& color, float thickness /*= 1.0f*/, float fade /*= 0.0f*/, int entityID)
	{
		CRYSTAL_PROFILE_FUNCTION();
	...
	void Renderer2D::DrawLine(const glm::vec3& startPoint, const glm::vec3& endPoint, const glm::vec4& color, int entityID /*= -1*/)
	{
	...
	void Renderer2D::DrawString(const glm::mat4& transform, const std::string& string, Ref<Font> font, const TextConfig& config)
	{
		...
		for (size_t i = 0; i < string.size(); i++)
		{
			...
			if (character == ' ')
			{
				...
				x += fsScale * advance + config.Kerning;
				continue;
			}

			if (character == '\t')
			{
				x += 4.0f * (fsScale * spaceGlyphAdvance + config.Kerning);
				continue;
			}
