...
namespace Crystal {
	...
	static std::unordered_map<std::string,ScriptFieldType> s_ScriptFieldTypeMap =
	{
		...
		{ "Crystal.Entity", ScriptFieldType::Entity },
	};
	...
	namespace Utils {
		...
		ScriptFieldType MonoTypeToScriptFieldType(MonoType* monoType)
		{
			...
			if (it == s_ScriptFieldTypeMap.end()) {
				CRYSTAL_CORE_WARN("Unknown Type Name!");
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		int32_t numTypes = mono_table_info_get_rows(typeDefinitionsTable);
		MonoClass* entityClass = mono_class_from_name(s_Data->CoreAssemblyImage, "Crystal", "Entity");
		...
		for (int32_t i = 0; i < numTypes; i++)
		{
			...
			/* This routine in an iterator routine for retrieving the fields in a class.
			   You must pass a gpointer that points to zero and is treated as an opaque handle
			   to iterate over all of the elements. When no more values are available, the return value is NULL */

			int fieldCount = mono_class_num_fields(monoClass);
			CRYSTAL_CORE_WARN("{} has {} fields:", className, fieldCount);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					CRYSTAL_CORE_WARN("  {} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
